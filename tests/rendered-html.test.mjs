...
test("product source contains the control-plane contract", async () => {
  ...
  assert.match(dashboard, /frontend-hyper-boost/);
  assert.match(dashboard, /Frontend Lab/);
  assert.match(dashboard, /Fork Blueprint/);
  assert.match(dashboard, /MASCOT/);
  assert.match(dashboard, /Presence shell/);
  assert.match(dashboard, /Accountable worker/);
  assert.match(dashboard, /Reusable capability/);
  assert.match(dashboard, /SKIPER 12/);
  assert.match(dashboard, /SKIPER 14/);
  assert.match(dashboard, /SKIPER 19/);
